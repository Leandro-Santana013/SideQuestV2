  ...
  try {
    ...
    <html>
      ...
      <body>
        ...
        <a href="http://localhost:5000/confirmar-email?token=${tokenConfirmacao}">Confirmar E-mail</a>
      </body>
    </html>
  `;
...
exports.validaEmail = async (req, res) => {
  try {
    ...
    if (tokenFromURL === tokenConfirmacao) {
      async function insertTokenIntoDatabase(user, token) {
        await queryAsync('UPDATE tb_cliente SET cd_token = ? WHERE cd_cliente = ?', [token, user]);
      }

      insertTokenIntoDatabase();

      return res.render('confirmarEmail', {
        message: 'E-mail confirmado com sucesso!'
      });
    } else {
      console.error('fudeu deu erro' + error)
      return res.render('confirmarEmail', {
